...
def  main(opt):
    ...
    if opt.load_reprs_path:
        for dataset, reprs_path in zip(opt.dataset, opt.load_reprs_path):
            ...
            cls1_instances[dataset] = reprs.loadh5file(reprs_path + '/' + f'{dataset}.{cls1_name}.{opt.focus}.h5')
            cls2_instances[dataset] = reprs.loadh5file(reprs_path + '/' + f'{dataset}.{cls1_name}.{opt.focus}.h5')
