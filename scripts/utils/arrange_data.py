...
def train_test_split(task, model, cls1_instances, cls2_instances, cls1_words, cls2_words, datasets, foci, is_lexical_split, is_random_labels, experiment_number, language=None, do_save=False):
    ...
    for dataset in datasets:
        for focus in foci:
            ...
            # save the splits:
            if do_save:
                with open(f'../splits/{task}/{model}/{language}/X_train_{dataset}_exp_{experiment_number}_{focus}.pkl', 'wb') as fout: pickle.dump(X_train[dataset][focus], fout)
                with open(f'../splits/{task}/{model}/{language}/Y_train_{dataset}_exp_{experiment_number}_{focus}.pkl', 'wb') as fout: pickle.dump(Y_train[dataset][focus], fout)
                with open(f'../splits/{task}/{model}/{language}/X_test_{dataset}_exp_{experiment_number}_{focus}.pkl', 'wb') as fout: pickle.dump(X_test[dataset][focus], fout)
                with open(f'../splits/{task}/{model}/{language}/Y_test_{dataset}_exp_{experiment_number}_{focus}.pkl', 'wb') as fout: pickle.dump(Y_test[dataset][focus], fout)

                with open(f'../splits/{task}/{model}/{language}/cls1_train_instances_{dataset}_exp_{experiment_number}_{focus}.pkl', 'wb') as fout: pickle.dump(cls1_train_instances[dataset][focus], fout)
                with open(f'../splits/{task}/{model}/{language}/cls2_train_instances_{dataset}_exp_{experiment_number}_{focus}.pkl', 'wb') as fout: pickle.dump(cls2_train_instances[dataset][focus], fout)
                with open(f'../splits/{task}/{model}/{language}/cls1_test_instances_{dataset}_exp_{experiment_number}_{focus}.pkl', 'wb') as fout: pickle.dump(cls1_test_instances[dataset][focus], fout)
                with open(f'../splits/{task}/{model}/{language}/cls2_test_instances_{dataset}_exp_{experiment_number}_{focus}.pkl', 'wb') as fout: pickle.dump(cls2_test_instances[dataset][focus], fout)
...
def load_splits(task, model, datasets, foci, experiment_number, language=None):
    X_train = {dataset: {} for dataset in datasets}
    X_test  = {dataset: {} for dataset in datasets}
    Y_train = {dataset: {} for dataset in datasets}
    Y_test  = {dataset: {} for dataset in datasets}
    ...
    for dataset in datasets:
        for focus in foci:
            logging.debug(f'../splits/{task}/{model}/{language}/X_train_{dataset}_exp_{experiment_number}_{focus}.pkl')
            with open(f'../splits/{task}/{model}/{language}/X_train_{dataset}_exp_{experiment_number}_{focus}.pkl', 'rb') as fin: X_train[dataset][focus] = pickle.load(fin)
            with open(f'../splits/{task}/{model}/{language}/Y_train_{dataset}_exp_{experiment_number}_{focus}.pkl', 'rb') as fin: Y_train[dataset][focus] = pickle.load(fin)
            with open(f'../splits/{task}/{model}/{language}/X_test_{dataset}_exp_{experiment_number}_{focus}.pkl', 'rb') as fin:  X_test[dataset][focus] = pickle.load(fin)
            with open(f'../splits/{task}/{model}/{language}/Y_test_{dataset}_exp_{experiment_number}_{focus}.pkl', 'rb') as fin:  Y_test[dataset][focus] = pickle.load(fin)

            with open(f'../splits/{task}/{model}/{language}/cls1_train_instances_{dataset}_exp_{experiment_number}_{focus}.pkl', 'rb') as fin: cls1_train_instances[dataset][focus] = pickle.load(fin)
            with open(f'../splits/{task}/{model}/{language}/cls2_train_instances_{dataset}_exp_{experiment_number}_{focus}.pkl', 'rb') as fin: cls2_train_instances[dataset][focus] = pickle.load(fin)
            with open(f'../splits/{task}/{model}/{language}/cls1_test_instances_{dataset}_exp_{experiment_number}_{focus}.pkl', 'rb') as fin:  cls1_test_instances[dataset][focus] = pickle.load(fin)
            with open(f'../splits/{task}/{model}/{language}/cls2_test_instances_{dataset}_exp_{experiment_number}_{focus}.pkl', 'rb') as fin:  cls2_test_instances[dataset][focus] = pickle.load(fin)
