...
class iNLP_Debiasing:
    ...
    def debias(self, X_train, Y_train, X_test, Y_test, opt, logfile_base):
        
        if not (opt.transfer_projmatrix or opt.transfer_classifier):

            for layer in range(self.N_LAYERS):
                logfile = open(f'{logfile_base}_layer-{layer}.txt', 'a') 
                iteration_accs = self.train(X_train[opt.train_dataset][opt.train_focus], 
                                             Y_train[opt.train_dataset][opt.train_focus], 
                                             X_test[opt.train_dataset][opt.train_focus], 
                                             Y_test[opt.train_dataset][opt.train_focus], 
                                             layer, 
                                             do_set_P=True)
            ...
            for layer in range(self.N_LAYERS):
                _ = self.train(X_train[opt.train_dataset][opt.train_focus], 
                               Y_train[opt.train_dataset][opt.train_focus], 
                               X_test[opt.test_on_dataset][opt.test_focus], 
                               Y_test[opt.test_dataset][opt.test_focus], 
                               layer, 
                               do_set_P=True)
        

        elif opt.transfer_projmatrix:

            for layer in range(self.N_LAYERS):
                logfile = open(f'{logfile_base}_layer-{layer}_original.txt', 'a') 
            
                print('Original Layer %d' % (layer+1))
                iteration_accs_ds1 = self.train(X_train[opt.train_dataset][opt.train_focus], 
                                                Y_train[opt.train_dataset][opt.train_focus], 
                                                X_test[opt.train_dataset][opt.train_focus], 
                                                Y_test[opt.train_dataset][opt.train_focus], 
                                                layer, 
            ...
            X_train_cleaned_test_dataset_test_focus = self.clean_data(X_train[opt.test_dataset][opt.test_focus])
            X_test_cleaned_test_dataset_test_focus  = self.clean_data(X_test[opt.test_dataset][opt.test_focus])
            ...
            for layer in range(self.N_LAYERS):
                ...
                iteration_accs_ds2 = self.train(X_train_cleaned_test_dataset_test_focus, 
                                                Y_train[opt.test_dataset][opt.test_focus], 
...
class BDD_Debiasing:
    '''Bias-Direction Debiasing, Dev and Phillips (2019) "Attenuating Bias in Word Vectorss"
